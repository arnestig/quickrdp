...
resourcesTabPanel::resourcesTabPanel(wxWindow* parent,wxWindowID id,const wxPoint& WXUNUSED(pos),const wxSize& WXUNUSED(size) )
{
...
void resourcesTabPanel::OnSoundClick(wxCommandEvent& WXUNUSED(event) )
{
    owner->checkForChanges();
}

void resourcesTabPanel::OnDiskMappingClick(wxCommandEvent& WXUNUSED(event) )
{
    owner->checkForChanges();
}
